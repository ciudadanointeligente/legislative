...
describe SearchesController do
  describe "GET index" do
    ...
    it "has a self reference" do
      ...
      assigns(:bills_query).self.should eq(ENV['billit'] + "search?&page=1&per_page=3&q=arica")
    end

    it "has a next page" do
      get :index, q: 'arica', per_page: '2', locale: 'es'
      assigns(:bills_query).next.should eq(ENV['billit'] + "search?&page=2&per_page=3&q=arica")
    end

    it "has a previous page" do
      get :index, q: 'arica', locale: 'es', page: 2
      assigns(:bills_query).previous.should eq( ENV['billit'] + "search?&page=1&per_page=3&q=arica")
    end
